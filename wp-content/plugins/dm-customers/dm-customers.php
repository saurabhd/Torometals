...
function dm_customers_add_to_cart ($cart) {	
	...
	if (!is_user_logged_in()) {
		echo apply_filters('dm_customers_login_to_add', $product->ID);
		return false;
	}
	...
	if (!user_can(get_current_user_id(), 'customer') && 
		...
		echo apply_filters('dm_customers_get_verified_to_add', get_current_user_id());
		return false;
	}
	return true;
}
...
add_filter('dm_customers_login_to_add', 'dm_customers_login_to_add', 50, 1);
...
add_filter('dm_customers_get_verified_to_add', 'dm_inv_get_verified_to_add', 5, 1);
...
// new unverified customer email
function dm_customers_add_email($emails) {
	$emails['WC_Email_New_Customer'] = include('class-wc-email-new-customer.php');
	return $emails;
}
add_filter( 'woocommerce_email_classes', 'dm_customers_add_email' );

function dm_customers_send_email($user_id) {
	$emails = WC()->mailer()->get_emails();
	$new_unverified = $emails['WC_Email_New_Customer'];
	$new_unverified->trigger($user_id);
}
add_action('user_register', 'dm_customers_send_email');
//add_action('woocommerce_created_customer_notification', 'dm_customers_send_email');
...
// woocommerce forces default role to be Customer - undo that
function dm_customers_undo_force_customer ($cust_data) {
	$cust_data['role'] = get_option( 'default_role' );
	return $cust_data;
}
add_filter('woocommerce_new_customer_data', 'dm_customers_undo_force_customer');
